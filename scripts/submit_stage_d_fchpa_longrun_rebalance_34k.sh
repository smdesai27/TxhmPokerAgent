#!/usr/bin/env bash
#SBATCH --job-name=ah_stage_d_rebalance34k_submit
...
#SBATCH --mem=1G
# Stage D rebalance long run: train -> screen -> gate -> cert
...
CONFIG_NAME="${CONFIG_NAME:-quadro_stage_d_fchpa_longrun_rebalance_34k_continue}"
STRICT_ABSTRACTION="${STRICT_ABSTRACTION:-true}"

SCREEN_EPISODES="${SCREEN_EPISODES:-5000}"
CERT_EPISODES="${CERT_EPISODES:-10000}"
BASE_SEED="${BASE_SEED:-42}"
HOLDOUT_SEED_BASE="${HOLDOUT_SEED_BASE:-100442}"
HOLDOUT_SEED_COUNT="${HOLDOUT_SEED_COUNT:-5}"
CI_FLOOR="${CI_FLOOR:-800.0}"

SNAPSHOT_PATH="${SNAPSHOT_PATH:-checkpoints/snapshots/stage_d/recovery/longrun_rebalance_34k_${STAMP}.pt}"
SCREEN_JSON="${SCREEN_JSON:-logs/stage_d/eval/eval_longrun_rebalance_34k_screen_${STAMP}.json}"
...
echo "Submitting Stage D rebalance 34k train..."
TRAIN_RAW="$(
  sbatch --parsable \
    --export=ALL,CONFIG_NAME="${CONFIG_NAME}",CONFIG_FILE="${CONFIG_FILE}",STRICT_ABSTRACTION="${STRICT_ABSTRACTION}",WANDB_RUN_GROUP=stage_d_longrun_rebalance_34k,WANDB_TAGS=stage_d,fchpa,rebalance34k,SNAPSHOT_COPY=true,SNAPSHOT_PATH="${SNAPSHOT_PATH}" \
...
SCREEN_RAW="$(
  sbatch --parsable \
    ...
    --export=ALL,CONFIG_NAME="${CONFIG_NAME}",CONFIG_FILE="${CONFIG_FILE}",CHECKPOINT_PATH="${SNAPSHOT_PATH}",STRICT_ABSTRACTION="${STRICT_ABSTRACTION}",EPISODES_PER_SEED="${SCREEN_EPISODES}",PROFILE=standard,BASE_SEED="${BASE_SEED}",HOLDOUT_SEED_BASE="${HOLDOUT_SEED_BASE}",HOLDOUT_SEED_COUNT="${HOLDOUT_SEED_COUNT}",VERIFY_SOLVER_TRAINING=true,REQUIRE_BEHAVIOR_EXTENDED=false,OUTPUT_JSON="${SCREEN_JSON}" \
...
GATE_RAW="$(
  sbatch --parsable \
    ...
    --export=ALL,EVAL_JSON="${SCREEN_JSON}",BASELINE_EVAL_JSON="${BASELINE_EVAL_JSON}",CI_FLOOR="${CI_FLOOR}",REQUIRE_BEHAVIOR_EXTENDED=false,REQUIRE_SOLVER_VERIFIED=true,REQUIRE_AGGRESSION_GATE=true,MAX_ALLIN_FREQ=0.05,MIN_CALL_CHECK_FREQ=0.40,MIN_HALF_POT_FREQ=0.01,MIN_RAISE_TOTAL_FREQ=0.08,MIN_FOLD_FREQ=0.35,MAX_FOLD_FREQ=0.55,MIN_ENTROPY_BITS=1.10,MAX_ENTROPY_BITS=1.80 \
...
Fallback submit examples (only if needed after screen/gate):
  # Condition A (EV strong, call/check still low):
  CONFIG_NAME=quadro_stage_d_fchpa_longrun_rebalance_34k_fallback_a \
    bash scripts/submit_stage_d_fchpa_longrun_rebalance_34k.sh

  # Condition B (EV below floor):
  CONFIG_NAME=quadro_stage_d_fchpa_longrun_rebalance_34k_fallback_b \
    bash scripts/submit_stage_d_fchpa_longrun_rebalance_34k.sh
EOF
